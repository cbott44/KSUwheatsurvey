...
producer_FILE_PATH = r"C:\Users\cb44\OneDrive - Kansas State University\Documents\Kansas State\Irrigated Wheat\Streamlitsurvey\data_collection_producers.csv"
...
path_file = producer_FILE_PATH
if not os.path.exists(path_file) or os.path.getsize(path_file) == 0: #if doesn't exist or is zero
    df = pd.DataFrame(columns=new_data.keys())
else:
    try:
        df = pd.read_csv(path_file)
    except pd.errors.EmptyDataError:                 #need this bc PD.read_csv doesn't know how to read empty file
        df = pd.DataFrame(columns=new_data.keys())
...
if add_data:
    ...
    df.to_csv(producer_FILE_PATH, index = False)
...
if not os.path.exists(field_FILE_PATH) or os.path.getsize(field_FILE_PATH) == 0:
    pd.DataFrame(columns=columns).to_csv(field_FILE_PATH, index=False)
else:
    try:
        df2 = pd.read_csv(field_FILE_PATH)
        
        if set(df2.columns) != set(columns):
            df2 = pd.DataFrame(columns=columns) #so that column names will work even if starting empty
    ...
    with placeholder.form("field1 Form",clear_on_submit = False):
        ...
        #------------------------------------------------------------------------------------------------#
            ...
            with st.expander("**If Soil Testing Prior to Planting;** provide details here"):
                ...
                left, right = st.columns(2, vertical_alignment = "bottom")
                new_data2['K_soil'] = left.text_input("Possasium (K) ppm")
            ...
            #saving and renaming uploaded files
            if uploaded_files is not None:
                number = 0
            
                for uploaded_file in uploaded_files:
                    
                    producer_id = st.session_state.get("producer_id", None) #pull from session state (form1)
        
                    # Extract file extension
                    file_extension = os.path.splitext(uploaded_file.name)[1]
        
                    # Create a new name
                    number = number + 1 #for form2 field will always be 1

                    #final naming convention
                    new_filename = f"soiltest{number}_{producer_id}_field1{file_extension}"
        
                    # Save to location
                    save_path = os.path.join(soil_tests, new_filename)
...
#add data       
    if dd_data:
        ...
        #other crop purpose 
            ...
            df2.to_csv(field_FILE_PATH, index = False)
    ...
    with st.form("Another Field",clear_on_submit = True):
        ...
        #------------------------------------------------------------------------------------------------#
            ...
            with st.expander("**If Soil Testing Prior to Planting;** provide details here"):
                ...
                left, right = st.columns(2, vertical_alignment = "bottom")
                new_data3['K_soil'] = left.text_input("Possasium (K) ppm", key = 'k2')
                ...
                if uploaded_files is not None:
                    number = 0
                    df2 = pd.read_csv(field_FILE_PATH) #always call this first 
                    if not df2.empty and 'field_number' in df2.columns:
                        last_field_numbertemp = df2['field_number'].iloc[-1]
                    else:
                        last_field_numbertemp = 1
                    field_numbtemp = last_field_numbertemp + 1           
                    for uploaded_file in uploaded_files:
                        ...
                        new_filename = f"soiltest{number}_{producer_id2}_field{int(field_numbtemp)}{file_extension}"
       ...
       #------------------------------------------------------------------------------------------------#
            ...
            new_data3['irr_decision'] = st.text_area("What drives your decision to trigger an irrigation event? (*ex: visual appearance of soil, moisture sensor*)", height = 4, key = 'irrdescision2')
            new_data3['irr_type'] = st.text_input("Irrigation Method (*i.e. center pivot, flood*)", key = 'irrtype2') 
            new_data3['system_config'] = st.text_area("Briefly describe the sprinkler configuration: spacing, height...", height = 4, key = 'system2')
...
#add data       
    if submit2:
        #grab last field number from above csv line and add 1
            df2 = pd.read_csv(field_FILE_PATH) #need to call it here

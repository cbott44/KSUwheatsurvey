#add column for date of submission
...
def read_csv_from_dropbox(dropbox_path, column_headers):
    try:
...
def write_csv_to_dropbox(df, dropbox_path):
    buffer = BytesIO()
...
df = read_csv_from_dropbox(PRODUCER_FILE_PATH, column_headers=new_data.keys())
